...
export default function AdminDashboard() {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        triggerStatus('success', 'Team account created successfully.');
        setNewUserName('');
        setNewUserEmail('');
        setNewUserPassword('');
        setNewUserRole('EDITOR');
        fetchContent(); // Refresh the users list
      } else {
        triggerStatus('error', data.error || 'Failed to create team account.');
      }
    } catch (err) {
      triggerStatus('error', 'Network error while creating account.');
    }
  };
  ...
  const handleDeleteUser = async (id: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        triggerStatus('success', 'User account deleted.');
        fetchContent(); // Refresh the users list
      } else {
        triggerStatus('error', data.error || 'Failed to delete user.');
      }
    } catch (err) {
      triggerStatus('error', 'Network error while deleting account.');
    }
  };
  ...
  const handleToggleContacted = async (id: string, currentStatus: boolean) => {
    try {
      ...
      const data = await res.json();
      if (data.success) {
        triggerStatus('success', 'Member status updated.');
        fetchContent(); // Refresh the members list
      } else {
        triggerStatus('error', data.error || 'Failed to update member.');
      }
    } catch (err) {
